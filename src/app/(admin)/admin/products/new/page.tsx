...
import { ArrowLeft, Image as ImageIcon, Plus, Trash2, Save, Sparkles } from "lucide-react";
...
export default function AdminNewProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast.success("New product dress added successfully to database!");
    ...
    <div className="max-w-4xl mx-auto space-y-8 font-body">
      ...
      <div className="space-y-1">
        <h1 className="text-2xl sm:text-3xl font-black font-heading text-white tracking-tight uppercase">
          Add New Dress Product
        </h1>
        <p className="text-xs sm:text-sm text-white/50 font-ui">
          Create a new style for the flat ₹999 combo catalog with custom images and size/color swatches.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="rounded-3xl border border-white/10 bg-[#161618] p-6 sm:p-8 space-y-6 shadow-xl">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className={`h-11 rounded-2xl font-bold text-xs cursor-pointer border transition-all ${gender === "men" ? "bg-amber-400 text-slate-950 border-amber-400" : "bg-white/5 text-white border-white/10"}`}
                >
                  Men's
                </button>
                <button
                  ...
                  className={`h-11 rounded-2xl font-bold text-xs cursor-pointer border transition-all ${gender === "women" ? "bg-amber-400 text-slate-950 border-amber-400" : "bg-white/5 text-white border-white/10"}`}
                >
                  Women's
                </button>
              </div>
            ...
            {/* Image URL */}
            <div className="space-y-1.5 sm:col-span-2">
              <label className="text-xs font-bold text-white uppercase tracking-wider block">Product Image URL</label>
              ...
              {variants.map((v, i) => (
                <div key={i} className="flex flex-wrap items-center gap-3 p-3 rounded-2xl bg-white/5 border border-white/5">
                  ...
                  <input
                    ...
                    className="h-9 px-3 w-28 bg-[#161618] border border-white/10 rounded-xl text-xs text-white"
                  />
        ...
        <div className="flex items-center justify-end gap-3">
          <button
            type="submit"
            disabled={loading}
            className="h-12 px-8 rounded-2xl bg-amber-400 text-slate-950 font-bold text-sm flex items-center gap-2 shadow-lg shadow-amber-400/20 hover:scale-105 transition-all cursor-pointer"
          >
            <Save className="h-4 w-4" />
            <span>{loading ? "Saving to Database..." : "Save Product to Database"}</span>
          </button>
        </div>
      </form>
    </div>
